...
class SoundFontPresetData:
	var phdr:Array[SoundFontPresetHeader]
	var pbag:Array[SoundFontBag]
	var pmod:Array[SoundFontModule]
	var pgen:Array[SoundFontGenerator]
	var inst:Array[SoundFontInstrument]
	var ibag:Array[SoundFontBag]
	var imod:Array[SoundFontModule]
	var igen:Array[SoundFontGenerator]
	var shdr:Array[SoundFontSampleHeader]
...
func _read_pdta_phdr( stream:StreamPeerBuffer ) -> Array[SoundFontPresetHeader]:
	var chunk:SoundFontChunk = self._read_chunk( stream, "phdr" )
	var phdrs:Array[SoundFontPresetHeader] = []
...
## @param	stream	ストリーム
## @return	bagデータのリスト
func _read_pdta_bag( stream:StreamPeerBuffer ) -> Array[SoundFontBag]:
	var chunk:SoundFontChunk = self._read_chunk( stream )
	var bags:Array[SoundFontBag] = []
...
func _read_pdta_mod( stream:StreamPeerBuffer ) -> Array[SoundFontModule]:
	var chunk:SoundFontChunk = self._read_chunk( stream )
	var mods:Array[SoundFontModule] = []
...
func _read_pdta_gen( stream:StreamPeerBuffer ) -> Array[SoundFontGenerator]:
	var chunk:SoundFontChunk = self._read_chunk( stream )
	var gens:Array[SoundFontGenerator] = []
	...
	# 4 bytes
	#while 0 < chunk_stream.get_available_bytes( ):
	for i in chunk_stream.get_available_bytes( ) / 4:
...
## @param	stream	ストリーム
## @return	楽器データのリスト
func _read_pdta_inst( stream:StreamPeerBuffer ) -> Array[SoundFontInstrument]:
	var chunk:SoundFontChunk = self._read_chunk( stream, "inst" )
	var insts:Array[SoundFontInstrument] = []
...
## @param	stream	ストリーム
## @return	波形データのリスト
func _read_pdta_shdr( stream:StreamPeerBuffer ) -> Array[SoundFontSampleHeader]:
	var chunk:SoundFontChunk = self._read_chunk( stream, "shdr" )
	var shdrs:Array[SoundFontSampleHeader] = []
